...
def pdf_converter():
    ...
    rendered = render_template("resume/index.html", user=user, edu=edu_info, inter=intership_info, work=work_info, prof=prof_skill, pers=pers_skill, hobby=hobby_skill)
    # os.path.join(app.config['APP_STATIC_ROOT'], 'static/css/bootstrap.min.css'), os.path.join(app.config['APP_STATIC_ROOT'], 'static/css/all.min.css'),
    css = [os.path.join(app.config['APP_STATIC_ROOT'], 'static/css/resume.css')]
